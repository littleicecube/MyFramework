 ...
 *
 *2^0=1个节点,从1开始数到1,在数组中从下标1到1						2^0=0+1;
 *2^1=2个节点,从1开始数到2,在数组中从下标2到3						2^1=2^0+1;
 *2^2=4个节点,从1开始数到4,在数组中从下标4到7						2^2=2^0+2^1+1;
 *2^3=8个节点,从1开始数到8,在数组中从下标8到15						2^3=2^0+2^1+2^2+1;
 *2^4=16个节点,从1开始数到16,在数组中从下标16到31					2^4=2^0+2^1+2^2+2^3+1;
 *2^5=32个节点,从1开始数到32,在数组中从下标32到63					2^5=2^0+2^1+2^2+2^3+2^4+1;
 *2^6=64个节点,从1开始数到64,在数组中从下标64到127					2^6=2^0+2^1+2^2+2^3+2^4+2^5+1;
 *2^7=128个节点,从1开始数到128,在数组中从下标128到255				2^7=2^0+2^1+2^2+2^3+2^4+2^5+2^6+1;
 *2^8=256个节点,从1开始数到256,在数组中从下标256到511				2^8=2^0+2^1+2^2+2^3+2^4+2^5+2^6+2^7+1;
 *2^9=512个节点,从1开始数到512,在数组中从下标512到1023				2^9=2^0+2^1+2^2+2^3+2^4+2^5+2^6+2^7+2^8+1;
 *2^10=1024个节点,从1开始数到1024,在数组中从下标1024到2047			2^10=2^0+2^1+2^2+2^3+2^4+2^5+2^6+2^7+2^8+2^9+1;
 *2^11=2048个节点,从1开始数到2048,在数组中从下标2048到4095			2^11=2^0+2^1+2^2+2^3+2^4+2^5+2^6+2^7+2^8+2^9+2^10+1;
 *2^12=4096个节点,从1开始数到4096	,在数组中从下标4096到				2^12=2^0+2^1+2^2+2^3+2^4+2^5+2^6+2^7+2^8+2^9+2^10+2^11+1;
 ...
 * （2）的内容：
 * 分配一个Chunk的大小，Chunk的长度有之前的参数决定，然后创建一个Chunk长度大小的数据，Chunk的长度作为数据的下标，下标作为向量使用数据的内容作为二叉数据的高度
 ...
 * 深度11:如果深度是11，那么将会有2^11==2048个page，每一个page都有对应的编号，因为11是最大深度，表示11下代表的就是
 *	 叶子节点，就是一个page，(默认情况下一个pageSize为2^13即8192)
 ...
 * 	######
	...
	如果id == 1024;	depth(id) == 10; log2PoolChunkSize-10 = 14; 1<< 14 == 8192*2是2个PageSize的大小
	如果id == 512 ; depth(id) == 9 ; log2PoolChunkSize-9  = 15; 1<< 15 == 8192*4是8个PageSize的大小
	...
	前提：
		一个PoolChunkSize == 2^24;大小的一维byte数组，表示一个高度11 == maxOrder,的完全二叉树；
	...
	######
	PoolChunk代表一颗完全二叉树，代表内存大小为16M,其中每个Page的Pagesize为8192那么Page的个数为
	this.subpages = 16M = 2^24 = 2^11*pageSize; 一共有2048个Page，那么每当分配一个Page后，就需要计算
	这个配置在subpages中的下标，以及Page在16M中的相对起始位置
	//根据异或，去除高位计算Page在subpages中的下标
	private int subpageIdx(int memoryMapIdx) {
        return memoryMapIdx ^ maxSubpageAllocs; 
    }
	1）根据id计算出所在层级，然后和层级的基地址进行异或计算出在层级中的偏移位置
	2）根据runLength(id)，根据id获取层级数就可以根据完全二叉树和PageSize的大小计算出一个层级下一个节点代表
		的叶子节点总数的大小
