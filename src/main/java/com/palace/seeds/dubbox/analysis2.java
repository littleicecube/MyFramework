...
在程序启动的过程中执行如下代码，就会将上述路径下配置的信息加载到loader实例中
ExtensionLoader loader = ExtensionLoader.getExtensionLoader(com.alibaba.dubbo.rpc.Protocol.class);
当要使用thrift协议时通过如下方法获取对应的实现
Protocol protocol = (Protocol) loader.getAdaptiveExtension();  
Exporter<?> exporter = protocol.export("thrift");

获取某个具体的协议实现时不会通过ExtensionLoader直接向外暴露出来，而是通过ExtensionLoader.getAdaptiveExtension()
对外暴露一个适配器，然后从适配器中获取具体的实现ExtensionLoader.getAdaptiveExtension().export("thrift")
每种类型的适配器可以预先写好配置在对应的路径下，如果没有预先实现的适配器则会通过字节码动态生成一个适配器，例如com.alibaba.dubbo.rpc.Protocol
的适配器类就是动态生成的代码如下：
...
class ExtensionLoader{
	//表示当前ExtensionLoader实例描述的是com.alibaba.dubbo.rpc.Protocol类型的信息
...
1）假设要将UserService中的add方法暴露出去对外提供服务，利用dubbo作为通信协议，创建一个url描述如下：
URL url = new URL("dubbo","127.0.0.1",8899);
url.addParameter("interface", "UserService")
.addParameterAndEncoded("methods", "add")
.addParameter("registry", "zookeeper")
2）将暴露的方法封装成一个invoker,将服务注册到服务中心zookeeper上
DubboInvoker invoker = new DubboInvoker<>(UserService.class,url);
3）创建一个Protocol.class类型的ExtensionLoader加载所有通信协议
ExtensionLoader loader = ExtensionLoader.getExtensionLoader(Protocol.class);
4）获取Protocol.class的适配器实例
Protocol protocol = (Protocol) loader.getAdaptiveExtension();
5)
Protocol.export(Invoker arg0){
		//从Invoker中获取协议
		com.alibaba.dubbo.common.URL url = arg0.getUrl();
		//默认情况下是Dubbo类型的协议
		String extName = url.getProtocol() == null ? "dubbo" : url.getProtocol() ;
