 ...
 * 
 逻辑地址空间:
 假设存在一个应用程序A
 经过编译后生成二进制代码,二进制代码一般会被划分为几个区域比如基本信息段,程序数据段,堆栈段,代码段
每一个段都是一个地址的集合,段地址中对应的信息可能是数据区的,也可能是代码区的
...
SEGMENT_BEGIN:
				...
				+----------+----------------------------------------+
				|000000005 |shr	eax, 16                             |
				+----------+----------------------------------------+
				|000000006 |mov	byte [LABEL_DESC_CODE16 + 4], al    |
				+----------+----------------------------------------+
				|000000007 |mov	byte [LABEL_DESC_CODE16 + 7], ah    |
				+----------+----------------------------------------+
...
如果为数据段,代码段,堆栈段都创建一个段描述符那么合起来就是一个段描述符表其中段基址+段内偏移得到是线性地址
逻辑地址是给应用程序看的,比如在c语言中&var获取的地址是逻辑地址,应用程序被编译连接后的到的文件从一端到另一端是一个逻辑地址空间其中
每一个指令在文件中的顺序可以看做是该指令的逻辑地址,所以所逻辑地址是给应用程序看的
有上可以知道编译连接后得到的文件都是有编排的,是按照段基址+偏移地址组织起来的,段基址+偏移地址是线性地址,线性地址是给cpu看的
